...
{
	std::cout << "Testing... " << '\n';
	std::vector<BingoBoard>* bingoBoards_test = parseBingoBoards(&strArrayBoards_test, 5, 5);
	day4_1_soln(&drawNumbers_test, bingoBoards_test);

	std::cout << '\n' << '\n' << "Doing real problem... " << '\n';
	std::vector<BingoBoard>* bingBoards = parseBingoBoards(&strArrayBoards, 5, 5);
	day4_1_soln(&drawNumbers, bingBoards);
}

void day4_1_soln(const std::vector<int>* dNumbers, std::vector<BingoBoard>* boards)
{
	int dNumbersSize = dNumbers->size();
	int bSize = boards->size();
	for (int i = 0; i < dNumbersSize; i++)
	{
		int value = dNumbers->at(i);
		for (int j = 0; j < bSize; j++)
		{
			BingoBoard board = boards->at(j);
			board.update(value);
			if (board.isWinner())
			{
				int sum = board.sumValsForBool(false);
				std::cout << "The winning value is: " << sum * value;
				return;
			}
		}
	}
...
void day4_2_soln()
{
	std::cout << "Testing... " << '\n';
	std::vector<BingoBoard>* bingoBoards_test = parseBingoBoards(&strArrayBoards_test, 5, 5);
	day4_2_soln(&drawNumbers_test, bingoBoards_test);

	std::cout << '\n' << '\n' << "Doing real problem... " << '\n';
	std::vector<BingoBoard>* bingBoards = parseBingoBoards(&strArrayBoards, 5, 5);
	day4_2_soln(&drawNumbers, bingBoards);
}

void day4_2_soln(const std::vector<int>* dNumbers, std::vector<BingoBoard>* boards)
{
	int dNumbersSize = dNumbers->size();
	for (int i = 0; i < dNumbersSize; i++)
	{
		...
		for (int j = boards->size()-1; j>=0; j--)
		{
			...
			{
				int sum = board.sumValsForBool(false);
				std::cout << "The winning value is of the final board is: " << sum * value;
				return;
			}
...
std::vector<BingoBoard>* parseBingoBoards(const std::vector<std::string>* boardsAsString, int width, int height)
{
	std::vector<BingoBoard>* retval = new std::vector<BingoBoard>();
	int size = boardsAsString->size();
	for (int i = 0; i < size; i++)
	{
		std::string boardStr = boardsAsString->at(i);
		retval->push_back(BingoBoard{ boardStr, width, height });
	}
	return retval;
}

BingoBoard::BingoBoard(std::string strArray, int width, int height)
{
	this->width = width;
	this->height = height;
	this->values = new std::vector<std::vector<std::pair<int, bool>>> ();
	parseStrArray(strArray);
}

void BingoBoard::parseStrArray(std::string strArray)
{
	int numAdded = 0;
	while (strArray.length() > 0)
	{
		...
		int value = std::stoi(strArray.substr(0, i));
		strArray.erase(0, i);
		int row = numAdded / width;
		int col = numAdded % width;

		if (col == 0)
		{
			values->push_back(std::vector<std::pair<int, bool>>{});
		}
		values->at(row).push_back(std::pair<int, bool>{value, false});
		numAdded++;
	}
}

std::pair<int, bool>* BingoBoard::valAt(std::pair<int, int> position)
{
...
void BingoBoard::update(int value)
{
	for (int row=0; row< height; row++)
	{
		for (int col = 0; col < width; col++)
		{
			std::pair<int, bool>* val = valAt(std::pair<int,int>{ row, col });
			if (val->first == value)
			{
				val->second = true;
			}
...
bool BingoBoard::colIsWinner(int col)
{
	for (int i = 0; i < height; i++)
	{
		if (!valAt(std::pair<int, int>{i, col})->second)
		{
			return false;
		}
	}
...
bool BingoBoard::rowIsWinner(int row)
{
	for (int i = 0; i < width; i++)
	{
		if (!valAt(std::pair<int, int>{row, i})->second)
		{
			return false;
		}
	}
...
int BingoBoard::sumValsForBool(bool marked)
{
	int retval = 0;

	std::vector<int>* valsForBool = getValsForBool(marked);
	int size = valsForBool->size();
	for (int i = 0; i < size; i++)
	{
		retval += valsForBool->at(i);
	}
	return retval;
}
std::vector<int>* BingoBoard::getValsForBool(bool marked)
{
	...
	for (int i = 0; i < height; i++)
	{
		...
		{
			std::pair<int, bool>* val = valAt(std::pair<int,int>(i, j));
